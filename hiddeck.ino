...
const int LIGHT_PIN = 13;        // Pin for controlling the light
const int RX_PIN = 0;            // RX pin (digital pin 0)
const int TX_PIN = 1;            // TX pin (digital pin 1)
const int SDA_PIN_CUSTOM = 2;    // Custom SDA pin (digital pin 2)

File myFile;

void setup() {
  pinMode(LIGHT_PIN, OUTPUT);        // Set the light pin as an output
  pinMode(RX_PIN, INPUT_PULLUP);     // Set RX pin as input with internal pull-up
  pinMode(TX_PIN, INPUT_PULLUP);     // Set TX pin as input with internal pull-up
  pinMode(SDA_PIN_CUSTOM, INPUT_PULLUP); // Set custom SDA pin as input with internal pull-up

  // Turn on the light when the device is plugged in
  ...
  // Determine which script to run based on pin state
  if (digitalRead(RX_PIN) == LOW) {
    runScript("script1.txt");
  } else if (digitalRead(TX_PIN) == LOW) {
    runScript("script2.txt");
  } else if (digitalRead(SDA_PIN_CUSTOM) == LOW) {
    runScript("script3.txt");
  } else {
    // Default case when no pins are connected to GND
    runScript("script0.txt");
  }

  // Turn off the light when setup is complete
